...
# %% ../../nbs/handlers/jois.ipynb #dde34b4e
lut_cols = {
...
class MeltJOISCB(PerGroupCB):
    ...
    def __init__(self,
        ...
        store_attr()
    def each_grp(self, grp, df, tfm):
        vals = df.melt(id_vars=self.meta_cols, value_vars=self.val_cols, var_name='nuclide_raw', value_name=self.val_name)
        uncs = df.melt(id_vars=self.meta_cols, value_vars=[f'unc_{c}' for c in self.val_cols], value_name=self.unc_name)
        uncs['nuclide_raw'] = uncs['variable'].str.replace('unc_', '', regex=False)
        out = vals.merge(uncs[self.meta_cols + ['nuclide_raw', self.unc_name]], on=self.meta_cols + ['nuclide_raw'])
