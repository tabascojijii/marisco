"""Docs: https://franckalbinet.github.io/mariscohandlers/pipeline/assembly.html.md"""
...
# %% auto #0
__all__ = ['resolve_callback', 'load_plugin', 'build_normalize_cbs', 'build_chain', 'build_phase_pipelines',
           'build_core_pipeline', 'run_preflight', 'run_finalize']

# %% ../../../nbs/handlers/pipeline/assembly.ipynb #1b3d4a5f
def resolve_callback(spec: PluginSpec, yaml_dir: Path = None):
    "Return the CB class for the given PluginSpec."
    return spec.resolve(yaml_dir)


def load_plugin(spec: PluginSpec, yaml_dir: Path = None):
    "Resolve a PluginSpec and instantiate it with spec.args."
...
def run_preflight(state: PipelineState, cfg: HandlerConfig, yaml_dir: Path = None) -> PipelineState:
    ...
    chain = [
        ...
        *[load_plugin(s, yaml_dir=yaml_dir) for s in cfg.pre_cbs],
        *pre_lossy,
    ]
...
def run_finalize(state: PipelineState, cfg: HandlerConfig, yaml_dir: Path = None) -> PipelineState:
    ...
    chain = [
        *post_lossy,
        *[load_plugin(s, yaml_dir=yaml_dir) for s in cfg.post_cbs],
    ]
    run_pipeline(state, chain)
    return state
