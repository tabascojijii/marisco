...
def _apply_measurement_defaults(state: PipelineState, cfg: HandlerConfig) -> None:
    "Apply declarative fallback values after mapping and melt, before Gate 2."
    for df in state.dfs.values():
        for column, value in cfg.measurement_defaults.items():
            if column in df:
...
def run_preflight(state: PipelineState, cfg: HandlerConfig, yaml_dir: Path = None) -> PipelineState:
    ...
    _apply_measurement_defaults(state, cfg)
