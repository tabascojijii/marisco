...
- The same semantic-exactness rule applies to acceptance-criteria identifiers such as `REQ-AC-...`; they are normative requirements, not a looser secondary class.
- For existence-style or state-style acceptance criteria, a cited downstream item is not direct evidence unless the item itself states that the required deliverable will exist or that the required state will be achieved. Naming only a file class, target directory, prerequisite constraint, or related design topic is insufficient.
...
- When a supporting-governance row or trace entry requires fielded content such as criterion, evidence path, or threshold, that fielded content must appear in the row or traced item itself rather than being inferred from neighboring rows, section defaults, or general document narrative.
- When structural governance defects are repaired in a revision set, authors and auditors must close missing authority-bearing semantics in `docs/requirements.md` and `docs/reference_standards.md` first, then realign `docs/plan.md`, `docs/roadmap.md`, and supporting-governance documents to that repaired upstream contract. A downstream document that looks exact on its own terms does not cure an upstream contract gap.
...
- When supporting governance documents are part of the active documentary-phase scope during a repair revision, they must be checked for same-change-set consistency with the repaired governing contract and active downstream text, while still remaining subordinate operationalization surfaces rather than replacement authority.
