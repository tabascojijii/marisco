...
- `REQ-GRAN-REQS-COMPLETE`: Requirements-level detail must include any condition that would otherwise make acceptance contradictory, undefined, or not auditable within the fixed audit scope. When a revision set is repairing structural governance defects, the governing two-document contract must be closed here first; downstream plan, roadmap, and supporting-governance edits must follow that repaired upstream text rather than compensating for an upstream gap locally.
...
- `REQ-GRAN-CHECKS`: Operationalized checks and pass thresholds belong in `docs/acceptance_matrix.md`. For every normative `REQ-...` identifier in this document, the matrix must define:
  ...
  Each matrix row must carry those fields in its own text; the matrix must not rely on section-level defaults, neighboring rows, or prose elsewhere to supply a missing field or threshold.
  ...
  If one requirement needs more than one downstream item to cover distinct required outcomes, each cited item must still directly state the portion it is claimed to satisfy; support documents must not rely on cross-item reconstruction to make the mapping appear complete.
...
- `REQ-CONTRACT-CLOSURE-PLAN`: `docs/plan.md` may require alignment work on supporting governance documents as follow-on execution work, but Architect gate success must remain decidable from the scoped governing contract and the plan itself rather than from out-of-scope supporting-document state. If the same revision set clarifies the governing contract upstream, the plan must consume that repaired present-tense contract directly and must not defer its own contract closure to a later upstream rewrite.
...
- `REQ-CONTRACT-CLOSURE-SUPPORT-SEPARATION`: Required supporting-governance documents may be revised in the same overall change set for consistency, but documentary-phase gate validity for `docs/plan.md` and `docs/roadmap.md` must not depend on first proving supporting-document completion unless those supporting documents are explicitly inside the fixed audit scope for that phase. When those supporting documents are inside the active fixed audit scope, they must be brought into same-change-set consistency with the repaired governing contract and downstream text, but that consistency obligation does not make them replacement authority.
- When supporting-governance documents are explicitly inside the active fixed audit scope, they are audited for consistency and completeness as subordinate operationalization surfaces only; they still must not backfill missing governing-contract rules or repair semantically invalid downstream citations by implication.
