...
| 層 | 主な役割 | 具体的に扱うもの | ここで止めるべきもの | ここで吸収・変換するもの |
|---|---|---|---|---|
| `Pydantic` | 境界契約の明示 | `configs.toml`、環境変数、CLI引数、Zotero応答の最低限の必須項目 | 必須設定欠落、CLI契約違反、外部API応答の必須項目欠損 | 原則として吸収しない |
| `handler` | provider固有データの読込とMARIS標準への翻訳 | CSV/Excel/URL入力、列構造、sample typeごとの分配、標準列への寄せ | 処理開始に必要な最低限の列が存在しない場合 | provider固有形式、列構造差、データセット固有の解釈 |
| `callback` | 局所的な補正・正規化・変換処理 | 列名変換、値補正、単位換算、欠損補完、座標変換、日時変換 | 局所変換が成立しない致命的不整合 | 列名ゆれ、値表記ゆれ、補正可能な欠損、正規化ルール |
| `encoder` | 正規化済みデータの出力 | NetCDF/CSV生成、global attrs付与、最終フォーマット化 | 出力に必要な構造が満たされない場合 | 原則として吸収しない |

この表で重要なのは、**Pydanticは「入口の契約」を守り、handler / callback は「現実データの揺れ」を引き受け、encoder は「整ったものを書き出す」** という分業である。
...
本提案でいう `fail fast` は、**入力データの揺れを早い段階で一律拒否すること**ではない。  
ここでの `fail fast` は、**境界契約を満たしていないものを、後段の変換ロジックに持ち込む前に明示的に止めること**を意味する。

対象となるのは、たとえば次のようなケースである。

- CLI引数の組み合わせが契約を満たしていない
- 必須設定が欠けている
- Zotero応答に最低限必要な項目が存在しない
- handlerが処理を始めるための最低限の列すら存在しない

一方で、次のようなものは `fail fast` の対象ではなく、引き続き handler / callback で吸収・補正する。
...
つまり本提案における `fail fast` は、**柔軟性の否定**ではなく、**柔軟性を適切な責務の中に閉じ込めるための入口整理**である。
...
本提案の `fail fast` には、次のような実務上のメリットがある。

- **原因の局所化**
  - 設定不備、CLI不備、外部API不備を、変換ロジックの不具合と切り分けやすくなる
- **後段デバッグの削減**
  - callback の深い場所で `KeyError` や `AttributeError` として落ちる前に止められる
- **エラーメッセージの改善**
  - 「何が足りないか」「どの契約に違反したか」を利用者向けに明示しやすくなる
- **再現性の向上**
  - 同じ不備が同じ入口で止まるため、調査・共有・修正がしやすい
- **責務分離の明確化**
  - 境界で止めるべきものと、handlerで吸収すべきものを整理しやすくなる
- **保守コストの削減**
  - “どこか後ろで壊れる” 事象が減り、変更時の影響把握が容易になる

特に本プロジェクトでは、外部データが一定程度「汚い」ことを前提にしているため、  
`fail fast` の価値は **厳格な拒否** にあるのではなく、**処理不能な問題だけを早めに表面化させること**にある。
